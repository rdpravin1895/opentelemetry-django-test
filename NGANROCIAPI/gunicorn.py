...
logconfig_dict = {
    ...
    'handlers': {
        'console': {
            'level': os.getenv('ROCI_LOG_LEVEL', 'INFO'),
...
def post_fork(server, worker):
    ...
    #     server.log.info("Opentelemetry enabled")
    #     resource = Resource.create(attributes={
    #         "service.name": os.environ.get("OTEL_SERVICE_NAME", "fds-api-service")
    #     })
    #
    #     span_processor = BatchSpanProcessor(
    #         OTLPSpanExporter(endpoint=os.environ.get("OTEL_COLLECTOR_ENDPOINT", "http://localhost:4317"))
    #     )
    #
    #     trace.set_tracer_provider(TracerProvider(resource=resource, active_span_processor=span_processor,
    #                                              id_generator=AwsXRayIdGenerator()))
    #     # trace.get_tracer_provider().add_span_processor(span_processor)
    # else:
    #     server.log.info("Opentelemetry not enabled")
